package com.gitee.ylx.util.doc;
...
public class DocUtil {
    ...
    //说明:生成升级脚本  server 服务名 , 扫包
    /**{ ylx } 2020/4/7 23:08 */
    public static void  updateSql(String server, String...scan) throws IOException {
        // 获取特定包下所有的类(包括接口和类)
            ...
            for (Class<?> cls : clsList) {
                ...
                if (annotation != null){
                    for (Method m : cls.getDeclaredMethods()){
                        ...
                        if (doc != null)
                        System.out.println("INSERT INTO `lltt_cloud_basic`.`sys_service_method` (`method_key`, `method_meaning`, `module_meaning`, `author`, `is_enabled`, `service_id`, `is_limit`, `no_more_call`) VALUES ('"+doc.method()+"', '"+doc.name()+"', '"+annotation.name()+"', '"+doc.auth()+"', '1', '"+server+"', '1', '0');");
                    }
                }
            }
        }
...
//        ClassPool pool = ClassPool.getDefault();//获取方法行数
//        Arrays.sort(methods, new Comparator<Method>() {
//            @Override
//            public int compare(Method o1, Method o2) {
//                try {
//                    return pool.getMethod(cls.getName(),o1.getName()).getMethodInfo().getLineNumber(0)
//                            -pool.getMethod(cls.getName(),o2.getName()).getMethodInfo().getLineNumber(0);
...
//        });
	...
	public void setTable(String id , String className ,String cName , String methodId , String methodName ,
		...
		for (String string : out) {
			...
			pw.println("                        <td class=\"font4\">"+ar[0]+"</td>");
			pw.println("                        <td class=\"font1\">"+(ar.length<2?"":ar[1])+"</td>");
